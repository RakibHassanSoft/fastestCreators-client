...
const FAQTabs = () => {
  ...
  const currentFAQ = faqData.find((faq) => faq.category === activeTab);

  return (
    <div className="w-full py-16 px-4 bg-gradient-to-br bg-whiteite to-green-900 text-white">
      <Header
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-10">
        {faqData.map((tab) => (
          <button
            ...
            className={`px-2 py-1 rounded-full font-semibold transition-all duration-300 border ${
              activeTab === tab.category
                ? "bg-green-900 text-white"
                : "bg-white text-black"
            }`}
          >
            {tab.icon} <span className="ml-1">{tab.category}</span>
          </button>
        ))}
      </div>
      ...
      <div className="max-w-4xl mx-auto space-y-2">
        {currentFAQ.questions.map((item, index) => (
          <div key={index} className="bg-white text-black rounded-lg shadow-md">
            ...
            <div
  className={`px-6 text-md text-gray-700 overflow-hidden transition-all duration-500 ease-in-out
    ${openFAQ === index ? "max-h-96 pb-4 opacity-100" : "max-h-0 pb-0 opacity-0"}`}
>
  {item.answer}
</div>
